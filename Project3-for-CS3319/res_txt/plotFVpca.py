...
file_path = "res_txt/res_FV_PCA.txt"
...
def plot_BOW(file_name):
    ...
    for i in range(len(lines)): 
        print(i)
        c = float(lines[i].split()[3].replace(',', ''))
        linear_score = float(lines[i].split()[5].replace(',', ''))
        rbf_score = float(lines[i].split()[7].replace(',', ''))
    ...
    plt.xlabel('k in FV_PCA')
    plt.ylabel('Accuracy of linear and rbf SVM model')
    plt.title('Scores of SIFT with FV_PCA based on Randm Sampled Dataset')
    plt.legend(bbox_to_anchor=(0.85, 0.95   ), loc='upper left')
    plt.savefig('Scores_of_SIFT_with_FV_PCA.jpg')
    plt.show()
